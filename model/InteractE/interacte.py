...
class Main(object):
	...
	def load_data(self):
		...
		for split in ['train', 'test', 'valid']:
			for line in open(f'{args.dataset_path}/{args.dataset_name}/{split}.txt'):
	...
	def fit(self):
		...
		val_mrr = 0
		save_path = f'{fpath}/torch_saved/{self.p.name}.model'
	...
	def enhance_side(self, split='valid', mode='tail_batch'):

		self.model.eval()
		enhance_data=pandas.DataFrame([],columns=['head','relation','tail','score'])
		with torch.no_grad():
			train_iter = iter(self.data_iter['{}_{}'.format(split, mode.split('_')[0])])
			for batch in tqdm.tqdm(train_iter,desc='测试进度'):
				...
				if 'tail' in mode:
					for h,r,sim_scores in zip(sub.cpu().numpy(),rel.cpu().numpy(),pred.cpu().numpy()):
						temp_data=pandas.DataFrame({'head':[h]*self.p.num_ent,'relation':[r]*self.p.num_ent,'tail':list(range(self.p.num_ent)),'score':sim_scores})
						temp_data=temp_data.nlargest(args.candidate_entity_limit,'score')
						enhance_data=pandas.concat((enhance_data,temp_data),axis=0)
				elif 'head' in mode:
					for t,r,sim_scores in zip(sub.cpu().numpy(),rel.cpu().numpy(),pred.cpu().numpy()):
						r-=self.p.num_rel
						temp_data=pandas.DataFrame({'head':list(range(self.p.num_ent)),'relation':[r]*self.p.num_ent,'tail':[t]*self.p.num_ent,'score':sim_scores})
						temp_data=temp_data.nlargest(args.candidate_entity_limit,'score')
						enhance_data=pandas.concat((enhance_data,temp_data),axis=0)

		return enhance_data

	def enhance(self, split):

		left_results  = self.enhance_side(split=split, mode='tail_batch')
		right_results = self.enhance_side(split=split, mode='head_batch')
		enhance_data=pandas.concat((left_results,right_results),axis=0,ignore_index=True)
		return enhance_data
...
def parser_arg():
	...
	parser.add_argument("--num_workers",	type=int,           default=4,                      		help='Maximum number of workers used in DataLoader')
	...
	# Logging parameters
	parser.add_argument('--logdir',    	dest="log_dir",       	default=f'{fpath}/log/',               		help='Log directory')
	parser.add_argument('--config',    	dest="config_dir",    	default=f'{fpath}/config/',            		help='Config directory')
	

	InteractE_args = parser.parse_args()
	InteractE_args.name=f'{args.dataset_name}_{InteractE_args.k_w}_{InteractE_args.k_h}'
	return InteractE_args

def fire():

	InteractE_args = parser_arg()
	set_gpu(InteractE_args.gpu)
	np.random.seed(InteractE_args.seed)
	torch.manual_seed(InteractE_args.seed)

	model = Main(InteractE_args)
	model.fit()
...
def enhance_data():
	...
	model = Main(InteractE_args)
	model.load_model(f'{fpath}/torch_saved/{model.p.name}.model')		
	enhance_data=model.enhance('train')

	enhance_data['head']=enhance_data['head'].map(lambda one:model.id2ent[one])
	enhance_data['relation']=enhance_data['relation'].map(lambda one:model.id2rel[one])
	enhance_data['tail']=enhance_data['tail'].map(lambda one:model.id2ent[one])
...
def is_model_exist():
	...
	InteractE_args = parser_arg()
	path=f'{fpath}/torch_saved/{args.dataset_name}_{InteractE_args.k_w}_{InteractE_args.k_h}.model'
